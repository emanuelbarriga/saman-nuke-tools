...
7. **ACL de la config central de render (`.saman/studio_config.json`)** — LucidLink: **WRITE solo admin** y **READ para los `ssh_user` de los workers de render** (mínimo grupo/máquinas render). Fallback por nodo: config completa en `config_local.py` (`RENDER_LOCAL_CONFIG`, gitignored) — autonomía local sin disco. Verificación: un usuario worker puede `cat {base}/.saman/studio_config.json`, o el nodo tiene `RENDER_LOCAL_CONFIG` completo. (Decisión D8 de `render-config-central`.)