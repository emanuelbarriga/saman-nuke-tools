...
The load MUST validate the schema before use: top-level `bases_por_so` (mapping OS→path), `workers` (list), `sufijos` (mapping); each worker MUST have `nombre`, `ssh` (host or None), `ssh_user`, `nuke_exec`, `base`, `lc_all`. Validation MUST accumulate ALL missing keys and type errors into one `SystemExit` listing each failure with repair guidance.
...
- GIVEN the first commit of `render_distribuido/`
